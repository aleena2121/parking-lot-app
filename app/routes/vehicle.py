...
from app.queries.vehicle_queries import (get_active_vehicles_in_lot, get_all_vehicles, get_vehicle_by_id,
...
@vehicle_router.get("/search", response_model=StandardResponse[List[ShowVehicle]])
def find_active_vehicle(
    ...
    return StandardResponse(
        message=f"{len(vehicles)} vehicle(s) found",
        payload=vehicles,
        status_code=status.HTTP_200_OK
    )
